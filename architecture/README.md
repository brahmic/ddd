...
Далее рассматривается вариант с изолированным ядром; он же принят за основу в [профиле Laravel](../laravel/README.md#lar-profile-01). DDD допускает разные способы организации приложения. Степень изоляции выбирается по [ARCH-FIT-01](../core/principles.md#arch-fit-01): приоритет сохраняется за явными границами, а упрощения требуют определённой области применения и оценки последствий для изменений и проверок.
...
Границы контекста, агрегата, транзакции и процесса определяются отдельно. Контекст задаёт область согласованной предметной модели; [агрегат](../core/domain-model.md#ddd-inv-01) объединяет объекты, целостность которых защищается совместно. Транзакция определяет атомарно фиксируемые изменения, процесс — среду исполнения кода. Один компонент может включать несколько агрегатов, несколько компонентов — работать в одном процессе и использовать общую БД. [Гарантии согласованности](../core/application.md#arch-cons-01) и размещение исполнения выбираются с учётом этих различий.
...
Порт описывает разговор с ядром на языке его потребностей. Входящий адаптер обращается к входному контракту; исходящий реализует контракт возможности, нужной ядру. Различие определяется тем, кто инициирует взаимодействие. Основание — [Hexagonal Architecture](https://alistair.cockburn.us/hexagonal-architecture); критерии принадлежности и формы контракта — [ARCH-PORT-01](../core/boundaries.md#arch-port-01).
...
При интеграции с другой предметной моделью может понадобиться ещё и перевод смысла: например, трактовка внешних статусов по правилам принимающего контекста. Это обязанность [Anti-Corruption Layer — DDD-ACL-01](../core/strategic-design.md#ddd-acl-01). Техническое преобразование формата само по себе не образует ACL. Обе обязанности могут выполняться одним адаптером, если их смысл и правила перевода остаются явными.

## Репозиторий, ORM и адаптер БД

[Контракт репозитория](../core/domain-model.md#ddd-repo-01) скрывает способ загрузки и сохранения предметных объектов. Если дополнительно выделен контракт `Persistence`, он изолирует потребителя операций хранения от API конкретного ORM. У этих абстракций разные обязанности. См. [Flow of control](https://herbertograca.com/2017/11/16/explicit-architecture-01-ddd-hexagonal-onion-clean-cqrs-how-i-put-it-all-together/#flow-of-control).
...
[Сценарий](../core/application.md#arch-use-01) — прикладная задача, например отмена брони. Прикладной сервис и обработчик команды — способы организовать её выполнение. `CancelBookingHandler` может сам реализовать сценарий; отдельный `CancelBookingService` между ним и моделью не является обязательной ступенью.
...
Возьмём намерение: «Отменить мою бронь» — из [сквозного примера методики](../core/workflow.md#сквозной-пример-отмена-бронирования). Для примера считаем, что владелец может отменить подтверждённую бронь строго до начала услуги. Повтор отмены уже отменённой брони успешен и не меняет состояние. Время решения определяется после загрузки брони.
...
Этот проход реализован в [примере на PHP и Laravel](../laravel/example.md); [структура каталогов](../laravel/structure.md#раскладка-сквозного-примера) показывает размещение его частей. Код охватывает только отмену брони. Возвраты, уведомления и Outbox в этом архитектурном разделе рассматриваются как возможные расширения, позволяющие разобрать дополнительные границы и гарантии.
...
[Доменное событие](../core/domain-model.md#ddd-event-01) сообщает о предметном факте, например `BookingCancelled` — об отмене брони. Прикладное событие сообщает об исходе целого сценария. Если эти смыслы совпадают, отдельные классы для обоих уровней не обязательны.
...
Доменный факт и внешнее сообщение могут иметь разные представления. Например, для доставки уведомления понадобятся идентификатор события и версия контракта. Их формат и правила повторной обработки согласуются на границе обмена. Критерии согласования фиксации, эффектов и повторов собраны в [ARCH-EFFECT-01](../core/application.md#arch-effect-01).
...
Порты позволяют подключать разные адаптеры к одному контракту и выполнять приложение в изоляции от внешних механизмов. Такую независимость проверяют на целом прикладном сценарии, включая его обращения к зависимостям. См. [Hexagonal Architecture](https://alistair.cockburn.us/hexagonal-architecture) и [TEST-USE-01](../core/testing.md#test-use-01).
...
Отдельно проверяется состав контрактов — это часть [проверки зависимостей](../core/testing.md#test-arch-01). Если `CancelBooking` принимает HTTP Request фреймворка, а `BookingStore` возвращает ORM Query Builder, технические зависимости уже проникли в ядро, даже при наличии интерфейсов и успешных тестов. В нашем примере контроллер передаёт прикладные идентификаторы и значения, а адаптер хранения возвращает предметную модель и оговорённые сведения о версии. Форматы на границе должны сохранять направление зависимостей; это также раскрыто в [What data crosses the boundaries](https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html#what-data-crosses-the-boundaries).

Изолированные проверки дополняются [проверками адаптеров](../core/testing.md#test-adapter-01) на реальных контрактах внешних механизмов. Хранилище в памяти не доказывает корректность SQL, отката транзакции или конкурентного обновления. При подмене целевой СУБД другой системой эти свойства также могут отличаться, поэтому их проверяют на целевой СУБД.
